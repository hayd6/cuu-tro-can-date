...
export default function MerchantOrdersClient({ orders }: { orders: Order[] }) {
  ...
  const renderOrderList = (list: Order[]) => {
    ...
    return list.map((order) => (
      <div
        key={order.id}
        className="bg-surface-container-lowest rounded-2xl p-5 shadow-[0_12px_32px_-4px_rgba(20,27,43,0.04)] relative overflow-hidden group border border-outline-variant/10"
      >
        {/* Urgency glow for pending */}
        {order.status === "CONFIRMED" && (
          <div className="absolute top-0 right-0 w-24 h-24 -mr-8 -mt-8 bg-tertiary-fixed/30 rounded-full blur-2xl group-hover:bg-tertiary-fixed/50 transition-all"></div>
        )}
  ...
  return (
    <div className="bg-surface text-on-surface min-h-[100dvh] pb-32 font-body">
      ...
      {/* Tabs */}
      <nav className="flex w-full bg-surface-container-low px-4 sticky top-16 z-40 border-b border-outline-variant/10">
        <div className="flex items-center justify-between w-full">
          {(
            ...
            >
              {label}
              {count > 0 && (
                <span className="bg-tertiary text-white text-[10px] px-1.5 py-0.5 rounded-full">{count}</span>
              )}
            </button>
          ))}
        </div>
      ...
      <Link
        ...
        className="fixed right-6 bottom-28 w-14 h-14 bg-gradient-to-br from-primary to-primary-container text-white rounded-full shadow-lg flex items-center justify-center active:scale-90 transition-transform z-40"
      >
